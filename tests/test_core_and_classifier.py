...
class CorePipelineTests(unittest.TestCase):
    ...
    def test_action_items_routes_to_reminders(self) -> None:
        ...
        ), patch.object(core, "classify", return_value=classify_result), patch.object(
            core, "add_reminder", return_value=True
        ):
    ...
    def test_action_items_raises_when_reminder_write_fails(self) -> None:
        ...
        with patch.object(
            core, "detect_content_type", return_value="action_items"
        ), patch.object(core, "classify", return_value=result), patch.object(
    ...
    def test_long_note_goes_to_notes(self) -> None:
        with patch.object(
            core, "detect_content_type", return_value="long_note"
        ), patch.object(core, "add_note", return_value=True):
            result = core.classify_and_route("long rambling journal entry...", source="iCloud")
        self.assertTrue(result.get("skip"))
    ...
    def test_unclear_creates_note_and_ref_reminder(self) -> None:
        with patch.object(
            core, "detect_content_type", return_value="unclear"
        ), patch.object(core, "add_note", return_value=True), patch.object(
            core, "add_reminder", return_value=True
        ):
    ...
    def test_row_id_calls_mark_failed_on_error(self) -> None:
        with patch.object(
            core, "detect_content_type", return_value="long_note"
        ), patch.object(core, "add_note", return_value=False), patch.object(
            core, "mark_failed"
        ) as mock_failed:
            with self.assertRaises(core.RoutingError):
...
class ClassifierFallbackTests(unittest.TestCase):
    ...
    def test_truncation_on_long_transcript(self) -> None:
        ...
        with patch.object(classifier, "log"), patch.object(
            ...
            post_mock.return_value.json.return_value = {
                "choices": [
                    {"message": {"content": '{"items": [{"item": "x", "category": "inbox"}]}'}}
                ]
            }
...
class DetectContentTypeTests(unittest.TestCase):
    ...
    def test_returns_unclear_on_api_failure(self) -> None:
        with patch.object(classifier.requests, "post", side_effect=Exception("network error")):
            result = classifier.detect_content_type("some text", api_key="key", model="model")
            self.assertEqual(result, "unclear")
    ...
    def test_returns_action_items_from_llm(self) -> None:
        with patch.object(classifier.requests, "post") as post_mock:
            ...
            result = classifier.detect_content_type("buy milk", api_key="key", model="model")
